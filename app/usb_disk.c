...
static FRESULT USB_DiskFileInfo(const char *path,FILINFO *filInfo);
static void USB_DiskRename(const char *path,const char *newPath);
static void USB_DiskSetFileDateTime(const char *path,uint8_t year,uint8_t month,uint8_t day,uint8_t hour,uint8_t min);
//static void USB_TaskLaunch(void);
...
static AppTask_S *FuncTask[] = {&HostEhci};
...
UsbDisk_S UsbDisk = {
    ...
    .fileInfo = USB_DiskFileInfo,
    .rename = USB_DiskRename,
    .setFileDateTime = USB_DiskSetFileDateTime,
    .mkdir = USB_Mkdir,
    .open = USB_Open,
    .write = USB_Write,
    .close = USB_Close,
...
static FRESULT USB_DiskFileInfo(const char *path,FILINFO *filInfo)
{
    FRESULT fResult = FR_NO_FILE;
    UsbCmd_S cmd = {(UsbCmdType_EN)0};
    uint16_t sLen;

    if(path == null || filInfo == null)
        return FR_NO_PATH;
...
//    FRESULT fResult;
    UsbCmd_S cmd = {(UsbCmdType_EN)0};
    uint16_t sLen;
	uint16_t yTmp = 2000 + year;

    if(path == NULL || year > 99 || month > 12 || day > 31 || hour > 23 || min > 59)
        return;

    if(USB_DiskFileIsExist(path)) {

        cmd.type = tSetDateTime;
		cmd.fInfo = MALLOC(sizeof(FILINFO));

		cmd.fInfo->fdate = ((yTmp - 1980) << 9 | month << 5 | day); /* 2015.10.1 */
    	cmd.fInfo->ftime = (hour << 11 | min << 5);               /* 12:30:00 */
 ...
 * @func 更新U盘信息
 */
static FRESULT USB_GetUsbDiskInfo()
{
    char *operaField;
    FATFS *fs;
...
//static void USB_TaskLaunch(void)
//{
//	Log.d("USB configuration finish ... \r\n");
//
